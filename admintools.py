    ...
    async def watcher_protections(self, message: Message):
        ...
        if (
            ...
            asyncio.ensure_future(
                self.p__bnd(chat, user, perms, message, bnd, bnd_sets)
            )
            await asyncio.sleep(0.5)
            asyncio.ensure_future(self.p__bcu(chat, user, message, bcu, bcu_sets))
            await asyncio.sleep(0.5)
            asyncio.ensure_future(
                self.p__bdl(chat, user, perms, message, bdl, bdl_sets)
            )
            await asyncio.sleep(0.5)
            asyncio.ensure_future(
                self.p__bss(chat, user, perms, message, bss, bss_sets)
            )
            await asyncio.sleep(0.5)
            asyncio.ensure_future(self.p__admin(chat, user, perms, message))
        return
